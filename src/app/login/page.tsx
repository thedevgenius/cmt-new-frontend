...
export default function LoginPage() {
	...
	const { step, isLoading, error, formattedPhone, isLoggedIn, user } = useAppSelector((state) => state.auth);
	...
	useEffect(() => {
		...
		} else if (isAuthenticated && isProfileCreated) {
			router.push("/");
		}
	...
	const onOtpSubmit = (e: React.FormEvent) => {
		...
		if (fullOtp.length < 4) {
			// You can dispatch a local error here if desired
			return;
		}
	...
	return (
		<div className="min-h-dvh flex flex-col">
			<div className="max-w-3xl mx-auto w-full flex flex-col">
				{step === "phone" && (
					<div className="SendOtp relative px-3 flex-1">
						...
						<div className="flex-1 flex flex-col items-center justify-center px-6 pb-6">
							...
							<p className="text-[15px] text-text-400 text-center max-w-62">
						...
						<div className="space-y-4">
							<form onSubmit={phoneForm.handleSubmit(onPhoneSubmit)} className="space-y-4">
								<div>
									<div className="flex items-center bg-slate-50 border border-slate-200 rounded-full h-14 px-2 mb-4 focus-within:border-primary-400 focus-within:ring-2 focus-within:ring-primary-50 transition-all">
				...
				{step === "otp" && (
					<div className="VerifyOTP">
						<header className="flex items-center shrink-0 mb-5">
							<button
								onClick={() => router.back()}
